...
class overbuff(tkinter.Tk):
    ...
    def get_info(self, gametag):
        ...
        my_url = 'https://playoverwatch.com/en-us/career/xbl/'+fuckfuckfuck.replace(' ', '%20')+'#competitive'
        try:
            uclient = ureq(my_url)
        except:
        ...
        player_info = {}
        player_info["name"] = page_soup.find("h1",{'class':'header-masthead'}).text
        heros = page_soup.find("div", {"id":"competitive"}).find_all("div",{"class":"progress-2 m-animated progress-category-item"})
        names = ['Mercy', 'Zenyatta', "Orisa", "Sombra", "Soldier: 76", "Moira", "Symmetra", "Roadhog", "Reaper", "Winston", "Bastion", "Tracer", "Lucio", "Hanzo", "Torjorn"]
        """
